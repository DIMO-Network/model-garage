...
// ToCurrentLocationCoordinates0 converts data from field 'drive_state.latitude' of type float64 to 'Vehicle.CurrentLocation.Coordinates' of type vss.Location.
// Vehicle.CurrentLocation.Coordinates: Current location of the vehicle in WGS 84 coordinates.
func ToCurrentLocationCoordinates0(originalDoc []byte, val float64) (vss.Location, error) {
	lonResult := gjson.GetBytes(originalDoc, "drive_state.longitude")
	if !lonResult.Exists() || lonResult.Value() == nil {
		return vss.Location{}, errors.New("latitude but no longitude")
	}
	lon, ok := lonResult.Value().(float64)
	if !ok {
		return vss.Location{}, fmt.Errorf("latitude present, but longitude had unexpected type %T", lonResult.Value())
	}
	return vss.Location{
		Latitude:  val,
		Longitude: lon,
